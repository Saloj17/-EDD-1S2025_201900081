...
public class visualizarServiciosUsuarios : Window
{
    private ListStore liststore;
    private TreeView treeview;
    private ComboBoxText orderCombo;

    public visualizarServiciosUsuarios() : base("Visualización de Servicios")
    {
        ...
        titleLabel.Markup = $"<span size='x-large' weight='bold'>Visualización de Servicios\n{Datos.blockchain.BuscarUsuarioId(Datos.idUsuarioLogin).Nombres}</span>";
    ...
    private void ConfigureColumns()
    {
        ...
        // Columna Vehiculo
        TreeViewColumn columnVehiculo = new TreeViewColumn("Vehículo", renderer, "text", 2);
        columnVehiculo.SortColumnId = 2;
        treeview.AppendColumn(columnVehiculo);
    ...
    private void LoadPostOrderData()
    {
        ...
        foreach (var item in Datos.serviciosArbol.PostOrdenList())
        {
            ...
            if (Datos.vehiculosLista.BuscarVehiculoId(item.Id_Vehiculo).Id_Usuario == Datos.idUsuarioLogin)
            {
                liststore.AppendValues(item.Id, Datos.repuestosArbol.BuscarPorId(item.Id_Repuesto).Repuesto, Datos.vehiculosLista.BuscarVehiculoId(item.Id_Vehiculo).Marca, item.Detalles, item.Costo.ToString("N2"));


                Console.WriteLine($"Id_usuario: {Datos.vehiculosLista.BuscarVehiculoId(item.Id_Vehiculo).Id_Usuario}");
                int usuarioId = Datos.vehiculosLista.BuscarVehiculoId(item.Id_Vehiculo).Id_Usuario;
                Console.WriteLine($"Usuario: {Datos.blockchain.BuscarUsuarioId(usuarioId).Nombres}\n");
            }
        }
    }
